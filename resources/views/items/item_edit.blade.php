            ...
            <div class="col-8 offset-2 bg-white">
                ...
                <form method="POST" action="{{ route('item.edit', [$item->id]) }}" class="p-5" enctype="multipart/form-data">
                            ...
                            @foreach ($categories as $category)
                                <optgroup label="{{$category->name}}">
                                    @foreach($category->secondaryCategories as $secondary)
                                        <option value="{{$secondary->id}}" {{$secondary->id == $item->secondary_category_id ? 'selected' : ''}}>
                                            {{$secondary->name}}
                                        </option>
                                    @endforeach
                                </optgroup>
                    ...
                    <div class="form-group mt-3">
                        ...
                        <select name="condition" class="custom-select form-control @error('condition') is-invalid @enderror">
                            @foreach ($conditions as $condition)
                                <option value="{{$condition->id}}" {{$condition->id == $item->item_condition_id ? 'selected' : ''}}>
                                    {{$condition->name}}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group mt-3" style="display: none">
                        <input id="id" type="number" name="id" value="{{$item->id }}" required autocomplete="id" autofocus>
                    </div>
